...
function SmartMapItem({ destination }) {
  const { onAddtoFavorite } = useDestination();
  const [searchParams] = useSearchParams();
  const lat = searchParams.get("lat");
  const lng = searchParams.get("lng");
  const isActive =
    Number(lat) === destination.position.lat &&
    Number(lng) === destination.position.lng;
  return (
    <div
    ...
    >
      <div className="relative  w-[inherit] h-4/5 ">
        ...
        <span
          className={`h-10 w-10 flex justify-center absolute top-2 right-2  items-center bg-neutraLight  transition-all duration-300 drop-shadow rounded-full cursor-pointer ${"bg-white"} hover:bg-white`}
          onClick={() => onAddtoFavorite(destination)}
        >
      ...
      <div
        ...
        }  py-3 px-2 rounded-b-lg text-neutraDark`}
      >
        <div className="flex items-start justify-between">
          <span className="flex flex-col gap-3">
            <p>5.7m ⭐⭐⭐⭐</p>
            <Link
              to={`/smart map?lat=${destination.position.lat}&lng=${destination.position.lng}`}
